...
-type(amqp_method_field_name() ::
       ( consumer_tag | ticket | class_id | method_id
       | arguments | delivery_tag | out_of_band | ticket
       | exchange | response | channel_id | no_local
       | active | prefetch_size | active | routing_key
       | global | reply_code | reply_text | arguments
       | class_id | method_id | ticket | no_ack
       | queue | queue | mandatory | consumer_tag
       | realm | exclusive | passive | active
       | no_ack | ticket | exchange | write
       | read | nowait | version_minor | ticket
       | mechanisms | locales | nowait | nowait
       | ticket | heartbeat | client_properties | mechanism
       | passive | locale | auto_delete | challenge
       | nowait | arguments | routing_key | channel_max
       | frame_max | version_major | exchange | channel_max
       | if_unused | nowait | frame_max | heartbeat
       | reply_code | ticket | reply_text | destination
       | source | routing_key | nowait | arguments
       | exchange | server_properties | reply_code | exchange
       | ticket | destination | source | routing_key
       | redelivered | routing_key | nowait | arguments
       | delivery_tag | redelivered | exchange | routing_key
       | virtual_host | immediate | message_count | ticket
       | exclusive | queue | passive | durable
       | prefetch_count | capabilities | exclusive | consumer_tag
       | auto_delete | nowait | arguments | delivery_tag
       | exchange | queue | insist | message_count
       | consumer_count | requeue | requeue | ticket
       | cluster_id | delivery_tag | queue | type
       | exchange | routing_key | nowait | consumer_tag
       | arguments | delivery_tag | response | multiple
       | known_hosts | requeue | ticket | queue
       | nowait | durable | message_count | ticket
       | consumer_tag | queue | routing_key | if_unused
       | requeue | if_empty | nowait | multiple
       | reply_text | message_count | internal | ticket
       | queue )).
