...
-type(amqp_method_field_name() ::
       ( routing_key | known_hosts | challenge | routing_key
       | nowait | queue | ticket | queue
       | global | consumer_tag | nowait | prefetch_count
       | ticket | cluster_id | routing_key | exchange
       | message_count | consumer_tag | mandatory | redelivered
       | immediate | consumer_tag | reply_code | ticket
       | integer_result | queue | multiple | if_unused
       | string_op | if_empty | nowait | consumer_tag
       | prefetch_count | routing_key | global | exclusive
       | message_count | passive | integer_2 | consumer_tag
       | string_result | delivery_tag | ticket | queue
       | reply_code | auto_delete | durable | routing_key
       | arguments | delivery_tag | no_ack | ticket
       | exclusive | nowait | frame_max | ticket
       | method_id | consumer_tag | identifier | exchange
       | delivery_tag | prefetch_size | reply_code | prefetch_count
       | exchange | delivery_tag | delivery_tag | heartbeat
       | result | prefetch_size | meta_data | nowait
       | consumer_tag | ticket | queue | routing_key
       | queue | integer_3 | consumer_tag | no_local
       | no_ack | requeue | exclusive | nowait
       | mandatory | ticket | channel_max | multiple
       | reply_code | no_local | consumer_tag | routing_key
       | ticket | exchange | consumer_tag | string_2
       | content_checksum | realm | nowait | message_count
       | no_ack | immediate | string_1 | identifier
       | reply_code | global | redelivered | reply_text
       | server_properties | requeue | class_id | operation
       | method_id | reply_text | exchange | redelivered
       | consumer_tag | routing_key | version_major | dtx_identifier
       | version_minor | delivery_tag | exclusive | mechanisms
       | locales | write | integer_4 | client_properties
       | ticket | mechanism | active | exclusive
       | ticket | exchange | locale | reply_text
       | consumer_tag | ticket | identifier | result
       | exchange | immediate | type | if_unused
       | passive | requeue | channel_max | auto_delete
       | internal | exchange | staged_size | nowait
       | reply_text | arguments | read | queue
       | queue | frame_max | nowait | heartbeat
       | ticket | exchange | prefetch_size | virtual_host
       | capabilities | response | insist | message_count
       | routing_key | known_hosts | integer_op | host
       | queue | mandatory | exchange | passive
       | response | exchange | reply_code | durable
       | table | integer_1 | reply_text | consume_rate
       | class_id | nowait | arguments | exchange
       | nowait | requeue | delivery_tag | consumer_tag
       | queue | content_size | reply_text | operation
       | out_of_band | queue | consumer_count | no_local
       | active | consumer_tag | ticket | delivery_tag
       | details | active | ticket | routing_key
       | exchange | exchange | routing_key | consumer_tag
       | nowait | arguments )).
